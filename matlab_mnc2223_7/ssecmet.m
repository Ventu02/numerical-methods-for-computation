...
%**************************************************
  ...
  x1=x0;
  fx1=feval(fun,x1);
  %finchè la differenza tra xstar e x1 è maggiore della tolleranza
  while ((abs(xstar-x1)>(tol+eps.*min([abs(x1),abs(xstar)]))) & (n<maxiter)); 
    ...
    if (abs(fx1)>=smallreal)
      xstar=x1-fx1*(x1-x0)./(fx1-fx0); %relazione del metodo delle secanti
      n=n+1;
      if(ftrace) %se è richiesto di ritornare i valori di successione
       xs(n)=xstar;
      end
    else
